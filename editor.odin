...
editor_render_level :: proc() /* imgui for selected level */
{
    ...
    if im.BeginCombo("Select Level", cfmt("%s", game.curr_level_id)) {
	for level_id in game.level_names do if im.Selectable(cfmt("%s", level_id)) do game.curr_level_id = level_id
	im.EndCombo()
    }
    //Select entity
    ...
    for flag in LevelFlagsEnum {
	if (flags_index % 2 != 0) do im.SameLine()

	flags_index += 1
	exits: bool = flag in level.flags

	if im.Checkbox(cfmt("%s", flag), &exits) {
	    if !exits do level.flags -= {flag}
	    else do level.flags += {flag}
	}
    }
    ...
    if im.ColorPicker4(
	"Background Color",
	    &level.background_color_f32,
	{.Uint8, .InputRGB, .PickerHueBar, .AlphaBar},
    ) {
    ...
    im.Text("Extra feilds")
    if game.extra_type_ids[.LEVEL] != nil {
	zipped := struct_fields_zipped_recursive(game.extra_type_ids[.LEVEL])
	for zip in zipped {
	    type := reflect.struct_tag_get(zip.tag, "imgui")
	    switch (type) {
	    case "vec2":
		val := cast(^[2]f32)(uintptr(&level.extra[0]) + zip.offset)
		im.SliderFloat(to_cstring(zip.name), &val.x, -1000, 1000)
		im.SliderFloat("Y", &val.y, -1000, 1000)
	    case "checkbox":
		val := cast(^bool)(uintptr(&level.extra[0]) + zip.offset)
		im.Checkbox(to_cstring(zip.name), val)
	    case "float32":
		val := cast(^f32)(uintptr(&level.extra[0]) + zip.offset)
		im.InputFloat(to_cstring(zip.name), val, 1)
	    case "b2Filter":

		filter := cast(^b2.QueryFilter)(uintptr(&level.extra[0]) + zip.offset)

		if im.BeginCombo("Category Bits", cfmt("%d", filter.categoryBits)) {
		    for type in EntityType {
			if im.Selectable(cfmt("%s", type)) do filter.categoryBits = u32(type)
		    }
		    im.EndCombo()
		}
		...
		for type in EntityType {
		    exits := bool(filter.maskBits & u32(type))
		    if im.Checkbox(cfmt("%s ", type), &exits) {
			if !exits do filter.maskBits ~= u32(type)
			else do filter.maskBits |= u32(type)
		    }
		}
	    }
	}
...
editor_render_asset_browser :: proc(def: ^CreateEntityDef = nil) {
	/* imgui asset picker for selected */
	col_index := 0
...
struct_fields_zipped_recursive :: proc(t: typeid, offset: uintptr = 0) -> []reflect.Struct_Field {
    ...
    for f in field {
	val, exists := reflect.struct_tag_lookup(f.tag, "imgui")
	if reflect.is_struct(f.type) && !exists {
...
editor_render_entity :: proc() {
	...
	if def.static_index != 0 {
	    ...
	    if im.BeginCombo("Select Level", cfmt("%s", game.editor_ctx.curr_static_index.level_id)) {
		for level in game.level_names{
		    if im.Selectable(cfmt("%d", level)) do game.editor_ctx.curr_static_index.level_id = level
		    game.editor_ctx.curr_static_index.index = 0
		}
		im.EndCombo()
	    }

	    //If no level is selected then current level is selected
	    if game.editor_ctx.curr_static_index.level_id == ""{
		game.editor_ctx.curr_static_index.level_id = game.curr_level_id
		game.editor_ctx.curr_static_index.index = 0
	    }

	    {
		selected_level := &game.levels[game.editor_ctx.curr_static_index.level_id]
		if im.BeginCombo("Select entity", cfmt("%d", game.editor_ctx.curr_static_index.index)) {
		    for key in selected_level.static_indexes {
			if im.Selectable(cfmt("%d", key)) do game.editor_ctx.curr_static_index.index = i32(key)
		    }
		    im.EndCombo()
		}
	    }
	...
	if game.extra_type_ids[.ENTITY] != nil {
		...
		case "checkbox":
		    ...
		    im.Checkbox(to_cstring(zip.name), val)
		case "float32":
		    val := cast(^f32)(uintptr(&def.extra[0]) + zip.offset)
		    im.InputFloat(to_cstring(zip.name), val, 1)
		case "b2Filter":
		    filter := cast(^b2.QueryFilter)(uintptr(&def.extra[0]) + zip.offset)
		    for type in EntityType {
			exits := bool(filter.maskBits & u32(type))
			if im.Checkbox(cfmt("%s ", type), &exits) {
			    if !exits do filter.maskBits ~= u32(type)
...
update_ctrl_keys :: proc() {
	...
	level := &game.levels[game.curr_level_id]
	if game.editor_ctx.multi_edit_mode {
	    //TODO: crashes on multiple entity because index of the entitydef changes after a single remove
	    for i in &game.editor_ctx.selected_entities {
		ordered_remove(&level.entity_defs, i - 1)

		for j, index in &game.editor_ctx.selected_entities{
		    if j >= i {
			game.editor_ctx.selected_entities[index] = j - 1
		    }
		}
	    }
	    clear(&game.editor_ctx.selected_entities)
